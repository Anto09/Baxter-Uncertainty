...
class BaxterJointUncertaintyCostGradientComputer
{
protected:
    ...
    sdf_tools::SDF_Builder sdf_builder_;
...
public:
    ...
    BaxterJointUncertaintyCostGradientComputer(ros::NodeHandle& nh, std::mt19937_64& prng, std::string& left_arm_points_dir, std::string& right_arm_points_dir) : nh_(nh), PRNG_(prng), model_(PRNG_)
    {
        ...
        sdf_tools::SDF_Builder builder(nh_, origin_transform, "base", 1.0, 2.0, 1.0, 0.05, 0.0, "get_planning_scene");
        sdf_builder_ = builder;
    ...
    {
        // Update the SDF from MoveIt's planning scene
        clock_t st, et;
        st = std::clock();
        sdf_ = sdf_builder_.UpdateSDF(sdf_tools::USE_FULL_PLANNING_SCENE);
        et = std::clock();
        ROS_INFO("SDF updated from MoveIt planning scene in %f seconds", (((float)(et - st)) / CLOCKS_PER_SEC));
#ifdef ENABLE_DEBUG_DISPLAY_MARKERS
        // Redraw the SDF marker
    ...
    {
        // Update the SDF if needed
        if (update_SDF)
        {
#ifndef DISABLE_SDF_RELOAD
            UpdateSignedDistanceField();
#endif
        }
    ...
    bool ImportInternalSDFFromFile(std::string& filepath)
    {
        ...
        return sdf_copy.LoadFromFile(filepath);
    }
};

int main(int argc, char** argv)
{
    ros::init(argc, argv, "baxter_cost_joint_uncertainty_server");
    ROS_INFO("Starting Baxter cost+joint uncertainty computation server...");
    ros::NodeHandle nh;
    ros::NodeHandle nhp("~");
    ...
    BaxterJointUncertaintyCostGradientComputer processor(nh, PRNG, left_arm_points_dir, right_arm_points_dir);
    // Save the internal SDF to file
    std::string filepath("/home/calderpg/Dropbox/ROS_workspace/src/Research/baxter_uncertainty/data/test_scene.sdf");
    bool saved = processor.ExportInternalSDFToFile(filepath);
    // Reload the sdf from file
    bool loaded = processor.ImportInternalSDFFromFile(filepath);
    if (loaded)
    {
        ROS_INFO("Reloaded internal SDF");
        processor.RedrawSignedDistanceField(0.05);
    }
    else
    {
        ROS_ERROR("Failed to reload internal SDF");
    }
    // Make service handler
    ros::ServiceServer feature_server = nh.advertiseService(feature_service, &BaxterJointUncertaintyCostGradientComputer::ComputeFeatureServiceCB, &processor);
    ros::ServiceServer gradient_server = nh.advertiseService(gradient_service, &BaxterJointUncertaintyCostGradientComputer::ComputeGradientServiceCB, &processor);
